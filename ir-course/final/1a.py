...
def main():
    ...
    docs = {x:y for x, y in zip(
        list(range(1, 11)), docs)}

    # initialize seed
    centroids = seed(docs, k)

    # perform clustering
    centroids, clusters = kmean(docs = docs, centroids = centroids, epochs = epochs)

    print('After 5 iterations, the centroids are {} and their clusters are {}'.format(centroids, clusters))
